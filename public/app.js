/* app.js — Dual-Pane Mapping with robust Drag&Drop (Drop Zones + Row-Half), EPG assign, server-side paging/search
   - Left: source channels (Xtream/M3U), Right: enabled channels
   - Drag from left→right inserts exactly at a drop zone or via row-half (before/after)
   - Drag within right reorders locally; persist with "Reihenfolge speichern"
...
async function enableChannel(id, dropIndex){
  try {
    ...
    // Sicherstellen, dass RIGHT_ROWS den Kanal hat
    await loadRight();
...
function buildRightDropZones(container){
  ...
  const makeZone = (idx)=>{
    ...
    // breitere Zonen, leichter zu treffen
  ...
  // Zone vor der ersten Reihe
  container.insertBefore(makeZone(0), rows[0] || null);
  // Zonen zwischen/unter den Reihen
  rows.forEach((row, i)=>{
...
function renderDualPane(){
  ...
  RIGHT_ROWS.forEach((ch) => {
    ...
    // NEW: Drop auf Zeile selbst (oben=vorher, unten=nachher)
    row.addEventListener('dragover', (e)=>{
      const rightHost = document.getElementById('rightList');
      // preview enlarged gap at computed index
      e.preventDefault();
